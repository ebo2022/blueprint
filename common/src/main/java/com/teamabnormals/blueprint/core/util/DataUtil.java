...
public final class DataUtil {
	 ...
	 *
	 * @param entries Pairs of an {@link Item} and a {@link Holder} of a String
	 */
	@SafeVarargs
	public static void registerDecoratedPotPattern(Pair<Item, RegistryHolder<String, ?>>... entries) {
		Map<Item, ResourceKey<String>> itemToPotTextureMap = Maps.newHashMap(DecoratedPotPatterns.ITEM_TO_POT_TEXTURE);
		for (Pair<Item, RegistryHolder<String, ?>> entry : entries) {
	...
	public static void changeBlockLocalization(String inputMod, String input, String outputMod, String output) {
		Block block = BuiltInRegistries.BLOCK.get(new ResourceLocation(inputMod, input));
		if (block != null)
	...
	public static void changeItemLocalization(String inputMod, String input, String outputMod, String output) {
		Item item = BuiltInRegistries.ITEM.get(new ResourceLocation(inputMod, input));
		if (item != null)
	 ...
	 * early as possible. On Forge, this can be done in an <code>enqueueWork</code> call within an <code>FMLCommonSetupEvent</code>.</p>
	 *
	 ...
	 * @author abigailfails
	 */
	//TODO: make these methods work with the common config system later
